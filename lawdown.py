...
def main(arguments):
    input_path = Path(arguments['<inputpath>'])
    output_path = Path(arguments['<outputpath>'])

    if not input_path.is_file():
        print("Input ist keine Datei!")
        return

    if input_path.suffix.lower() != ".xml":
        print("Input ist keine XML-Datei!")
        return

    print("Starte Konvertierung:", input_path)

    with input_path.open("r", encoding="utf-8") as infile, \
         output_path.open("w", encoding="utf-8") as outfile:
        law_to_markdown(infile, outfile, name=arguments['--name'])
